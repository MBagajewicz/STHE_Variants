...
def Exhaustive_Enumeration_Next_Level(OF_NAME, constraint_lists, var_list, OF_VAR, candidates , problem_data, 
                                      Type_Equipment, Active_Models_Constraints, Selected_Model, Selected_Example,
                                      save_result):
    ...
    for i in range(0, number_of_candidates):
        ...
        if not constraint_lists:
            ...
            SOL_NEXT_LEVEL[i] = Solution_Next_Level
        else:
            Feasibility = False
    ...
    for cont, value in zip(var_list, candidates[:, min_index]):
        sol_data[cont] = value
    sol_data['Next_Level'] = SOL_NEXT_LEVEL[min_index]
